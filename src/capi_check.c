...
int GracieExtract(void **GracieCtx, char *Text, unsigned int nTextBytes, unsigned int **Result,
        unsigned int *n32sCopied);
...
int main()
{
    ...
    unsigned int *Result;
    unsigned int n32sCopied;
    if (GracieExtract(&GracieCtx, Text, sizeof(Text), &Result, &n32sCopied) == GRACIE_SUCCESS)
    {
        for (int WCharIndex=0; WCharIndex < n32sCopied; ++WCharIndex)
            putwchar(((wchar_t *)Result)[WCharIndex]);
        putwchar(L'\n');
