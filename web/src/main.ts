...
function GenJSONConfig(S: gracie_state): string
{
    ...
    for (const ExtrDefItem of ExtrDefsContainer.children)
    {
        ...
        const CountryInput = TryGetElementByClassName(
            ExtrDefItem, "extr-country-input", 0) as HTMLInputElement;
        const LanguageInput = TryGetElementByClassName(
            ExtrDefItem, "extr-language-input", 0) as HTMLInputElement;
...
RunExtractor.onclick = () =>
{
    ...
    TryPUTExtractorOut(TA.innerText, ConfName).then(ExtractorOut =>
    {
        ...
        //DEBUGDisplayConfig.innerText = ExtractorOut.toString();
